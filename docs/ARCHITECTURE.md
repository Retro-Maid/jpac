...
src/jp_address_crosswalk/
  ...
  cli.py                jpac build|validate|verify|diff|export|baseline
  pipeline.py           the build, stage by stage (§2)
  config.py             typed config loading
  logging_setup.py      structlog JSON logging
  payload.py            accepted-payload record + safe archive reading (§4), DBF/SHP readers
  ...
  sources/
    ...
    mlit_ksj_n02.py     V2: 国土数値情報 N02 鉄道 (optional)
    estat.py            V2: e-Stat 国勢調査 小地域境界 (optional)
  build/
    ...
    busstop.py          V2: バス停留所 → 市区町村 (a published point; no representative point)
...
config/       matching_rules.yml, address_normalization.yml, sources.yml,
              candidate_sources.yml (sources under review), expected_schema/
overrides/    manual_overrides.yml, municipality_lineage.yml
identity/     address_id_ledger.csv.gz          (committed)
tools/        verification scripts behind `jpac verify`; build_diagrams.py;
              build_mesh_table.py / build_site_geo.py (the map's data, outside `jpac build`)
site/         the static map (POLICY.md §3.2): index.html, app.js, lookup.js, geo.js,
              data/ (committed, generated by the two tools above), open-local.bat + serve.ps1
tests/        unit, integration, fixtures; the site/ JavaScript is tested under node
```
...
`data/raw/`, `data/cache/`, `dist/` are gitignored, and so is everything in `site/data/`
except the files the page loads (`mesh_data.js`, `geo/`). Their *metadata* — URL, SHA-256,
...
One optional extra exists for a tool outside the release pipeline:

| Extra | Package | Why |
|---|---|---|
| `geo` | `shapely>=2.1,<3` | `tools/build_site_geo.py` only — the static map's boundary lines (`docs/POLICY.md` §3.2). It dissolves 231,661 小地域 into ~1,900 municipalities and simplifies them without opening gaps between neighbours (`coverage_union_all` / `coverage_simplify`). Those are GEOS algorithms; the ray cast and grid in `build/spatial.py` were enough for containment, but a correct polygon union and coverage-preserving simplification are not ~30 lines. `jpac build` never imports it, so the release pipeline's dependency set is unchanged. |
