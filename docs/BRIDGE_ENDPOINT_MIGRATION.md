...
§2 は 2026-09-24 に決定済み（フェーズ0 完了）。以降のフェーズは未着手で、ゲートを
飛ばさない。
...
| **1** | `build/common.py` の共通スキーマを端点列に置き換え、7本のビルダーを直す（`bridge_municipality_postal` の分割を含む） | フィクスチャビルドが通り、`finalize_bridge` の重複排除キーが端点列で成立する |
| **2** | `CHECK`（A5 の強い形）と `FOREIGN KEY` を書き換え、`PRIMARY_KEYS` / `SORT_KEYS` / 品質ゲートに新しい表を登録し、`docs/schema.sql` を更新 | `PRAGMA foreign_key_check` が全国データで違反0、`jpac verify artifacts` がオブジェクト一致（37テーブル） |
| **3** | `build_flat_view` の読み出し元を端点列に変える | **フラット表の43列が1列も変わらない**こと（バイト比較） |
| **4** | 互換ビュー7本（A7）を `DDL_VIEWS` に足す | 旧い名前のビューが旧い列名で同じ行数を返す |
| **5** | `docs/queries/` 5本・README・`DB_SCHEMA.md` §5.1 の「What is actually shipped」注記・`LIMITATIONS.md` 項目8 を更新 | 12本すべて実行でき、空を返さない |
| **6** | 全国ビルド + 2回ビルドのバイト一致 + v2.0.0 のリリースノート（**互換ビューは v2.1.0 で落とすと明記**） | 品質ゲート全通、`verify idempotent` 0 problem |

**G1（フェーズ1のゲート）で確かめるべき実測**: `bridge_municipality_postal` の
8,207 + 1,910 が、**2つの表に分かれて合計が変わらない**こと（8,207 + 1,910 = 10,117）。
ここで行が増えたり減ったりするなら、移行が「多態を解いた」のではなく「データを作り
変えた」ことになる。

**G2 で増える確認**: 新しい `bridge_municipality_postal_code` は行数変化のゲートから
見ると**前リリースに存在しない表**である。`compare_with_previous` は前が無い表を
skip するので赤にはならないが、`bridge_municipality_postal` のほうは 10,117 → 1,910 の
**-81.1%** になる。これは承認が要る（`config/quality_thresholds.yml` の
`approved_row_count_changes` に、人の署名つきで正確な行数を書く）。分割による移動で
あって欠損ではないことを、署名者が確かめられる形にしておく。
