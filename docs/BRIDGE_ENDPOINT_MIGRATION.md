...
§2 は 2026-09-24 に決定済み（フェーズ0）。**フェーズ1〜4 と6の実装は同日に完了し、
ゲートはすべて実測で通っている**（§3）。残っているのは A8 の署名と、v2.0.0 の公開
そのものである。
...
### A8. 分割で `candidate_count` の意味が変わる ← **署名が必要**
...
| **6** | ✅ 全国ビルド（`build: PASS 1.3.0+data-2026-09-17`、177ゲート 0 failed、37テーブル）／ 版を 2.0.0 に上げた再ビルドとリリースノートは **A8 の署名待ち** | ✅ 品質ゲート全通 |
