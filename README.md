...
<p align="center">
  ...
  データ：Parquet 35.2 MB ／ CSV.gz 41.3 MB ／ SQLite 1.85 GiB</sub>
</p>
...
`v2.0.0+data-2026-09-17`（727,110行）の実測値です。**v2.0.0 でブリッジの列名が変わりましたが、
この43列は1列も変わっていません。**
...
> 以下はすべて `v2.0.0+data-2026-09-17` の実測値です（品質チェック180項目すべて合格）。
...
来歴の記録として `source_snapshot` 115 件（取得したファイル1本 = 1行。URL・SHA-256・規約・
行数）、`match_run` 1 件、`match_run_input` 112 件。合計31テーブル。
...
Parquet 35.2 MB ／ CSV.gz 41.3 MB ／ SQLite 1.85 GiB ／ テーブル別 Parquet 31本。
...
リリースのタグはコード版とデータ版を併記します（`v2.0.0+data-2026-09-17`）。同じコードでも
...
- 版番号はコード版とデータ版を分離しています（`v2.0.0+data-2026-09-17`）。
