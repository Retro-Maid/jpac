...
`v1.1.0+data-2026-09-16`（727,110行）の実測値です。
...
> 以下はすべて `v1.1.0+data-2026-09-16` の実測値です（ビルド日時 2026-09-16T06:35:53Z ／
> 品質チェック124項目すべて合格）。数値は版が変わると変わります。
...
| 駅 ↔ 市区町村 | 9,047 | 99.96%（9,046駅のうち4駅が未確定） | 8,987 |

駅のブリッジだけ様子が違います。ほかは名前やコードの照合ですが、これは境界データの面に点が
入るかどうかの判定なので、確定した行は `auto` になります。要確認は60行で、内訳は旧浜松市の
区コード54行（境界データが2020年時点のため）、府県境をまたぐ大阪空港2行、どの境界にも
入らない4行です。
...
- 版番号はコード版とデータ版を分離しています（`v1.1.0+data-2026-09-16`）。
