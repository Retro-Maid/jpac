...
| `bridge_line_municipality` | 3,380 | 路線 ↔ 市区町村。1,444市区町村、最多は東海道新幹線の97 |
...
バス停のある市区町村は1,797。どの境界にも入らない115件は最寄りに寄せず `unresolved` のまま
...
- **jpac のデータも出します。** 判定した市区町村の**駅**（クリックしたメッシュ内のものと、
  市区町村内のものを分けて）・**郵便番号**・**市外局番**を同じパネルに表示します。駅は地図にも
  ピンで出ます（ズーム12以上、または市区町村を選んだとき）。鍵はすべて `lg_code` で、
  町字には降りません。市外局番が番号区画の一部しか占めない場合は、発行元の但し書きを
  そのまま添えます。
...
python tools/build_site_data.py                      # 駅・郵便番号・市外局番 → site/data/jpac_data.js
```
...
`dist/parquet/` の jpac のテーブルと `data/raw/mlit_ksj_n02/`（駅の形状）を読みます。
境界データと駅の代表点を `site/` で配布するのは
