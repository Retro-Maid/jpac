...
<p align="center">
  <a href="https://retro-maid.github.io/jpac/"><b>🗺️ 地図で見る</b></a> ・
  <a href="https://github.com/Retro-Maid/jpac/releases/latest"><b>⬇️ データをダウンロード</b></a>
</p>

<p align="center">
  <sub>地図：クリックした地点の市区町村・緯度経度・メッシュコードと、その市区町村の駅・郵便番号・市外局番が出ます（ブラウザだけで動きます）<br>
  データ：Parquet 35.2 MB ／ CSV.gz 41.3 MB ／ SQLite 1.95 GiB</sub>
</p>
...
[ライセンス](#ライセンス) ・ [駅と地図](#駅と地図v110-で追加) ・ [静的マップ](#静的マップ) ・
[リポジトリ構成](#リポジトリ構成)
...
`v2.0.1+data-2026-09-17`（727,110行）の実測値です。**v2.0.0 でブリッジの列名が変わりましたが、
この43列は1列も変わっていません。** v2.0.1 の修正（市区町村 ↔ 市外局番）もこの表の値は変えていません。
...
v1.1.0 で加わった対応です。どれも市区町村までで、町字（`address_id`）には結びません
...
`jpac build` が次の3テーブルを出力します（v1.0.0 の28テーブル → v1.1.0 の31テーブル。
路線とバス停を加えた v1.2.0 は36、`bridge_municipality_postal` を2つに分けた v2.0.0 は
37テーブルです）。
...
### 路線 → 市区町村 ／ 駅 → 路線（v1.2.0 で追加）

同じ N02 と同じ境界から、路線の対応も出します。**新しい出典は増えていません** ——
アダプタは以前から路線を読んでいて、テーブルにしていなかっただけです。
...
2m まで同じ答えになるため 25m）。40の市区町村を通る路線は「40行の正しい答え」であって
曖昧さではないので、駅と違い一意性は要求しません。
設計と測定: [`docs/RAILROAD_LINE_PLAN.md`](docs/RAILROAD_LINE_PLAN.md)。

### バス停留所 → 市区町村（v1.2.0 で追加）

国土数値情報のバス停留所データ（P11、**令和4年度版**）を、同じ e-Stat の境界で市区町村に
結びます。
...
（グループ内の最大距離は中央値16.4km、129組は県をまたぐ）。名前で集約すると遠く離れた別の
バス停を1行に融合してしまうため、`p11_stop_id` は「行の識別子」としての代理キーにしています
（`address_id` のような実体の同一性ではありません）。

ライセンスは版で分かれます。**令和4年度版のみ**を使います。平成22年度版は「非商用」で複製物の
再配布が除外されており、N07バスルートも全版が同じ条件なので、どちらも対象外です
（[`DATA_LICENSE.md`](DATA_LICENSE.md)）。

設計と測定: [`docs/BUS_STOP_PLAN.md`](docs/BUS_STOP_PLAN.md)。
...
- **jpac のデータも出します。** 判定した市区町村の**駅**（クリックしたメッシュ内のものと、
  市区町村内のものを分けて）・**通る路線**・**バス停の数**・**郵便番号**・**市外局番**を
  同じパネルに表示します。鍵はすべて `lg_code` で、町字には降りません。市外局番が番号区画の
  一部しか占めない場合は、発行元の但し書きをそのまま添えます。
- **地図に出るピン:** 駅はズーム12以上（または市区町村を選んだとき）、バス停はズーム14以上。
  バス停は27万件あるので全部を最初に読ませず、見えている2次メッシュのぶんだけ都度読みます
  （1チャンク中央値 2.1 KB）。同じ地点・同じ名称のものは1つのピンにまとめ、事業者は
  ツールチップに全部出します（実測で32,446行ぶんが同じ点に重なっていました。まとめるのは
  表示のためで、事業者は捨てていません）。
...
バス停の座標はリリースデータには入らないので、parquet ではなく生データから読みます。

境界データ・駅の代表点・バス停の点を `site/` で配布するのは
[`docs/POLICY.md`](docs/POLICY.md) §3.2 の例外で、リリースデータには入りません。
...
> 以下はすべて `v2.0.1+data-2026-09-17` の実測値です（品質チェック180項目すべて合格）。
> 数値は版が変わると変わります。最新値はリリースデータに同梱される `QUALITY_REPORT.md`
> にあります。データ版 `+data-2026-09-17` は**元データを取得した日**で、ビルド日では
> ありません（[`docs/ACQUISITION_DATES.md`](docs/ACQUISITION_DATES.md)）。

約72万件の住所を中心に、12万件の郵便番号、19万件の国土交通省コード、582件の番号区画、
9千件の駅を繋いでいます。市区町村は1,918。
...
末尾3本が0件なのは欠損ではなく、**元データがまだ一度も更新されていないため**です
（取得は 2026-08-23、以降の全リリースが同じ payload から作られています）。v2.0.0 以降、
この3本はリリースをまたいで積まれます —— 一度記録した事象は次のリリースでも残ります
（[`docs/LIMITATIONS.md`](docs/LIMITATIONS.md) 項目7）。
...
| `telephone_area_coverage` | 1,640 | 「この区画はどの市区町村を含むか」の公式文を1文ずつ構造化 |
| `telephone_number_block` | 41,987 | 市内局番の割り当て |
| `n02_station` | 9,046 | 駅（駅グループ単位）。国土数値情報 鉄道データ N02 2025年版 |
| `estat_small_area` | 232,019 | e-Stat 国勢調査 小地域の属性。**形状は含みません** |
...
| 市区町村 ↔ 郵便番号（変換表の市区町村レベル行） | 8,207 | 100% | 0 |
| 市区町村 ↔ 日本郵便の特殊レコード | 1,910 | 100% | 0 |
| 市区町村 ↔ 市外局番 | 2,153 | 100%（うち候補どまり 1.068%） | 0 |
| 駅 ↔ 市区町村 | 9,061 | 99.96%（9,046駅のうち4駅が未確定） | 9,027 |

市区町村 ↔ 郵便番号が2行あるのは、**v2.0.0 で表を2つに分けた**からです（`P2` は郵便番号を
直接指し、`P3` は日本郵便のレコードを指すので、同じ列に入れると join の前に規則で分岐する
必要がありました）。合計は分割前と同じ 10,117 です。

駅のブリッジだけ様子が違います。ほかは名前やコードの照合ですが、これは境界データの面に点が
入るかどうかの判定なので、確定した行は `auto` になります。要確認は34行で、内訳は旧北区の
14駅（中央区と浜名区に分割され承継先が一意でないため、候補2つで28行）、府県境をまたぐ
大阪空港2行、どの境界にも入らない4行です。

来歴の記録として `source_snapshot` 116 件（取得したファイル1本 = 1行。URL・SHA-256・規約・
行数）、`match_run` 1 件、`match_run_input` 113 件。合計37テーブル。
...
| 国土交通省 | 25.3% | 低いのは欠損ではなく粒度の違い。国土交通省側から見れば 183,382 / 191,106 = 96% が繋がっている |
| 市外局番 | 99.7% | 市区町村単位で 1,912 / 1,918。未達は北方領土の6件のみ |
| 駅 → 市区町村 | 99.96% | 9,042 / 9,046 駅。未達の4駅は埋立地や水域で、最寄りには寄せていません |
...
Parquet 35.2 MB ／ CSV.gz 41.3 MB ／ SQLite 1.95 GiB ／ テーブル別 Parquet 37本。
1枚にまとめた表（Flat View）は 727,110行 × 43列です。1つの郵便番号が複数行になるため、
行数は住所の件数より多くなります。43列は住所・郵便番号・国土交通省・市外局番の4系統で、
駅と境界の3テーブルはこの表には入りません（SQLite か テーブル別 Parquet で引いてください）。
...
リリースのタグはコード版とデータ版を併記します（`v2.0.1+data-2026-09-17`）。同じコードでも
...
![v1.0.0 の28テーブルのER図。実体と版、6本のブリッジ、来歴](docs/diagrams/04-er.svg)

<sub>図の出典: [`docs/diagrams/04-er.mmd`](docs/diagrams/04-er.mmd)</sub>

この図は v1.0.0 の28テーブルのものです。v1.1.0 で加わった3テーブル（`n02_station` /
`bridge_station_municipality` / `estat_small_area`）と、v1.2.0 で加わった5テーブル
...
- 版番号はコード版とデータ版を分離しています（`v2.0.1+data-2026-09-17`）。
