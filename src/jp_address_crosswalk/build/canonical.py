...
def _lineage_id(row: dict) -> str:
    ...
    **同じイベントが再発すると、記録されるのは最初の1回だけである**（レビューで
    指摘された）。A→B（第N版）、B→A（N+1）、A→B（N+2）と往復すると、N+2 の id は
    N と同じになり、``accumulate_events`` は前の行を残すので3回目は記録されず、
    残った行の ``observed_at`` は N を指したままになる。``observed_at`` を材料に
    入れれば区別できるが、そうすると台帳を持たない genesis ビルドが同じイベントを
    別の日付で再検出したときに行が二重になる —— 起きる確率と実害を比べて、
    **二重にしないほうを選んでいる**。往復した改名の回数を数える必要が出たときは、
    ``address_history``（値を材料に含む）のほうが答えを持っている。
    """
    material = "|".join(
        [
